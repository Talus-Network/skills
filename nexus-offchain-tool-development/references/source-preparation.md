...
Use this reference after the embedded contract/design gate and the off-chain HTTP Tool route. The pinned three-archive workflow, manifest schema, and cleanup implementation are documented in the commands and guidance below.

Set `SKILLS_BUNDLE_ROOT` to the installed bundle root. Prepare only `nexus-sdk`, `nexus-move-packages`, and `sui` with `scripts/prepare_sources.py`, then resolve `SDK_ROOT`, `MOVE_PACKAGES_ROOT`, and `SUI_ROOT` from the manifest through `root --manifest ... --repo ...`. Use only repository-relative paths under those verified roots; never require another checkout.

Install cleanup before preparation and run the bundle-root cleanup command exactly once for an existing manifest on normal exit or `INT`/`TERM`. Preserve the primary status and report cleanup failures on stderr. Stop at a failed prepare/root resolution instead of using an unverified fallback.

Public source and read-only Testnet GraphQL evidence explain interfaces and returned state only. They do not prove provider invocation, signed-result acceptance, registration, or payment. Keep local mock/invocation evidence, metadata validation, and any separately authorized network operation distinct.
